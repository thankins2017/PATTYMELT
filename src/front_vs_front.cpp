// Determines the front vs. front relationship from HDBSCAN clustering.
...
int main(int argc, char **argv) {
    // option manager
    BrAppOptionManager::Instance()->SetVersion(1, 0, "front_vs_front");
    BrAppOptionManager::Instance()->SetHelp("Calibrates target testing station DADL front vs. front.");
    ...
    std::cout << std::endl;
    std::cout << "front_vs_front : reduced file read-in" << std::endl;
    ...
    auto *g_FrontSumDif = new TGraph(); g_FrontSumDif->SetTitle("SumDif;Difference (chan);Sum (chan)");

    // Vector of Points to be passed to HDBSCAN clusterer
    std::vector<Point *> points {};

    // Front versus front event loop
    std::cout << "front_vs_front : entering event loop" << std::endl;
    ...
    std::cout << "front_vs_front : number of points passed to clusterer - " << number_clustered << std::endl;

    // Display the results
    auto *can = (TCanvas *)gROOT->FindObjectAny("can");
    if(!can) can = new TCanvas("can", "", 750, 750);
    g_FrontSumDif->Draw("ap");
    ...
    std::cout << "front_vs_front : initializing PCA objects" << std::endl;
    ...
    std::cout << "front_vs_front : performing PCA fits" << std::endl;
    std::vector<TF1 *> fits {};
    ...
    std::fstream out_file(Form("%s%s", CALIB_FILE_DIR, "front_vs_front.dat"), std::ios_base::out);
    out_file << f_final->GetParameter(0) << "\t" << f_final->GetParameter(1) << std::endl;
    out_file.close();

    in_file->Close();

    std::cout << "front_vs_front : finished" << std::endl;
    std::cout << std::endl;
